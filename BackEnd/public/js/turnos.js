...
document.getElementById('confirm-btn').onclick = async () => {
  ...
  try {
    ...
    // ✅ FIX: leer como texto primero y luego intentar parsear
    // Evita que res.json() explote si la respuesta viene vacía o mal formada
    let data = {};
    ...
    if(text){
      try {
        ...
        // El servidor respondió algo que no es JSON válido
        console.error('Respuesta inesperada del servidor:', text);
      }
    }

    if(!res.ok){
      showAlert(data.error || 'Ocurrió un error al reservar el turno.', 'error');
      return;
    }
    ...
    // Este catch es SOLO para errores de red reales (sin internet, timeout, CORS, etc.)
    console.error('Error de red:', e);
    showAlert('Error de conexión. Verificá tu internet e intentá de nuevo.', 'error');
  } finally {
    btn.disabled    = false;
